...
const ProductDetail = ({ productId }) => {
  ...
  return (
    <>
      ...
      <section className='flex gap-2 items-center mt-32'>
        <Link to="/">Home</Link>
        <div className='h-[8px] aspect-square bg-red-500 rounded-full'></div>
      ...
      <section className='grid gap-6 sm:grid-cols-2 sm:items-center max-w-[1000px] mx-auto'>
        ...
        <section className='overflow-hidden relative'>
          ...
          <i onClick={nextImage} className='bx bxs-right-arrow absolute top-1/2 -translate-y-1/2 left-2 text-red-600 right-2 hover:text-red-400  cursor-pointer'></i>
          <i onClick={previusImage} className='bx bxs-left-arrow absolute top-1/2 -translate-y-1/2 text-red-600 right-2 hover:text-red-400  cursor-pointer'></i>
        </section>
        ...
        <section>
          ...
          <button onClick={handleClickAddToCart} className='w-full bg-red-500 py-2 text-white hover:bg-red-600 transition-colors mt-6 rounded-sm'>add to cart <i className='bx bx-cart'></i></button>
      ...
      <SimilarProducts categoryId={productData?.categoryId} productId={productData?.productId} />
    </>
  )
}
